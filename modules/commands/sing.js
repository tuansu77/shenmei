...
let datab = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${value.id}&key=${keyapi}`)).data;
  ...
  let time = (gettime.slice(2));
        ///////////////////
  let datae = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=statistics&id=${value.id}&key=${keyapi}`)).data;
  let getlike = datae.items[0].statistics.likeCount;
  let getdislike = datae.items[0].statistics.dislikeCount;
        let datac = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${value.id}&key=${keyapi}`)).data;
  let channel = datac.items[0].snippet.channelTitle;
